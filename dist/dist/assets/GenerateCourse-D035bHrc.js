import{u as F,r,j as e,B as p,d as A,A as V}from"./index-C89U1syM.js";import{A as $}from"./index-DG7lZl3d.js";const G=()=>{const m=F(),g=5,[l,i]=r.useState([{sub:""}]),[o,b]=r.useState("4"),[c,j]=r.useState("Text & Image Course");r.useState(!1);const[y,n]=r.useState(!1);r.useState(""),r.useState([]),r.useState(0);const h=s=>{b(s.target.value)},x=s=>{j(s.target.value)},N=()=>{m("/viewcourse")};let w=(s,t)=>{let a=[...l];a[s][t.target.name]=t.target.value,i(a)},v=()=>{l.length<g?i([...l,{sub:""}]):p.error("You can only add 5 sub topics")},T=()=>{let s=[...l];s.pop(),i(s)};const C=s=>{s.preventDefault();const t=[];n(!0),l.forEach(u=>{t.push(u.sub)});const a=document.getElementById("topic1").value;if(!a.trim()){n(!1),p.error("Please fill in all required fields");return}if(t.length===0){n(!1),p.error("Please fill in all required fields");return}const d=`Generate a list of Strict ${o} topics and any number sub topic for each topic for main title ${a.toLowerCase()}, everything in single line. Those ${o} topics should Strictly include these topics :- ${t.join(", ").toLowerCase()}. Strictly Keep theory, youtube, image field empty. Generate in the form of JSON in this format {
        "${a.toLowerCase()}": [
   {
   "title": "Topic Title",
   "subtopics": [
